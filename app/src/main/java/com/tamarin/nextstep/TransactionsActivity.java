...
public class TransactionsActivity extends AppCompatActivity {
        ...
        RetrofitClient.getApi().getTransactions().enqueue(new Callback<List<Transaction>>() {
            ...
            public void onResponse(Call<List<Transaction>> call, Response<List<Transaction>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    applyFilters(); // Aplica os filtros assim que os dados chegam
        ...
        RetrofitClient.getApi().getCategories().enqueue(new Callback<List<Category>>() {
            ...
            public void onResponse(Call<List<Category>> call, Response<List<Category>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    categoryNames.add("Todas"); // Opção padrão
                    for (Category cat : categories) {
                        categoryNames.add(cat.getName());
                    }

                    ArrayAdapter<String> catAdapter = new ArrayAdapter<>(TransactionsActivity.this, android.R.layout.simple_spinner_item, categoryNames);
    ...
    private void setupFilters() {
        // Deteta quando o utilizador escreve no campo de busca
        ...
        // Deteta mudança no Spinner de Tipo
        ...
        // Deteta mudança no Spinner de Categoria
    ...
    private void applyFilters() {
        String textFilter = etSearch.getText().toString().toLowerCase().trim();
        String typeFilter = spinnerFilterType.getSelectedItem() != null ? spinnerFilterType.getSelectedItem().toString() : "Todos";
        String categoryFilter = spinnerFilterCategory.getSelectedItem() != null ? spinnerFilterCategory.getSelectedItem().toString() : "Todas";
        ...
        for (Transaction tx : allTransactions) {
            ...
            // Se a transação passar nos 3 filtros, é adicionada à lista visual
